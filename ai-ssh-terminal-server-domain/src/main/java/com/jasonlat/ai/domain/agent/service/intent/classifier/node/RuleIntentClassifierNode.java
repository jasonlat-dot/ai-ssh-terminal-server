 ...
 */
@Slf4j
@Component("ruleIntentClassifierNode")
public class RuleIntentClassifierNode extends AbstractIntentClassifierSupport {
     ...
     */
    private static IntentRuleVO rule(IntentTypeEnumVO intent, List<String> keywords, List<String> patterns) {
     ...
     */
    @Override
    public IntentResultVO doApply(IntentRequestVO requestParameter, DefaultClassifyFactory.DynamicContext dynamicContext) throws Exception {
        ConversationContextVO context = contextTracker.getContext(requestParameter.getChatSessionId());
        IntentResultVO ruleIntentResult = classify(requestParameter.getUserMessage(), context);
        dynamicContext.setFinalResult(ruleIntentResult);
    ...
    public StrategyHandler<IntentRequestVO, DefaultClassifyFactory.DynamicContext, IntentResultVO> get(IntentRequestVO requestParameter, DefaultClassifyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        if (finalResult.getConfidence() >= 0.8) {
            log.info("rule 用户识别意图结果：意图类型：{} | 置信度:【{} >= 0.8】 | 路由到 --> endIntentClassifierNode", finalResult.getIntent(), finalResult.getConfidence());
            return getBean("endIntentClassifierNode");
        }
        log.info("rule 用户识别意图结果：意图类型：{} | 置信度:【{} < 0.8】 | 路由到 --> llMIntentClassifierNode", finalResult.getIntent(), finalResult.getConfidence());
        return getBean("llMIntentClassifierNode");
    }
}
