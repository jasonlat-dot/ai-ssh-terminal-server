...
@Slf4j
@Service
public class AppNameValidateNode extends AbstractValidateSupport {

    @Resource
    private AgentIdValidateNode agentIdValidateNode;
    ...
    protected Boolean doApply(List<AiAgentConfigTableVO> requestParameter, DefaultValidateFactory.DynamicContext dynamicContext) throws Exception {
        ...
        for (AiAgentConfigTableVO aiAgentConfigTableVO : requestParameter) {
            ...
            if (StringUtils.isBlank(appName)) throw new AppException("appName is null, please check your yml config!");
            if (appIds.contains(appName)) throw new AppException("appName is repeated, please check your yml config!");

            appIds.add(appName);
        }
        // agent id 非空 & 重复检验通过
        log.info("AppNameValidateNode validate success!");
