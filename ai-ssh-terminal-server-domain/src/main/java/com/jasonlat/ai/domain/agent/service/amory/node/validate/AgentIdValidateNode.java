...
@Slf4j
@Service
public class AgentIdValidateNode extends AbstractValidateSupport {

    @Resource
    private AgentNameValidateNode AgentNameValidateNode;
    ...
    protected Boolean doApply(List<AiAgentConfigTableVO> requestParameter, DefaultValidateFactory.DynamicContext dynamicContext) throws Exception {
        List<String> appIds = new ArrayList<>(requestParameter.size());
        for (AiAgentConfigTableVO aiAgentConfigTableVO : requestParameter) {
            String appId = aiAgentConfigTableVO.getAgentDefinition().getAgentId();
            if (StringUtils.isBlank(appId)) throw new AppException("appId is null, please check your yml config!");
            if (appIds.contains(appId)) throw new AppException("appId is repeated, please check your yml config!");

            appIds.add(appId);
        }
        // agent id 非空 & 重复检验通过
        log.info("AgentIdValidateNode validate success!");
