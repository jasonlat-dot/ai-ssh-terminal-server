...
@Slf4j
@Service
public class AgentYmlConfigValidateNode extends AbstractValidateSupport {

    @Resource
    private AppNameValidateNode appNameValidateNode;
    ...
    protected Boolean doApply(List<AiAgentConfigTableVO> requestParameter, DefaultValidateFactory.DynamicContext dynamicContext) throws Exception {
        if (null == requestParameter || requestParameter.isEmpty()) {
            throw new Exception("AI Agent yml config is empty");
        }
        log.info("AgentYmlConfigValidateNode validate success!");
