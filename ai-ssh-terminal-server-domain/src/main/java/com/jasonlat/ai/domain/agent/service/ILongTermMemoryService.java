 ...
 * <ul>
 *   <li>{AiCallNode} 首轮调用 {@link #recordUserMessage} 提取用户偏好</li>
 *   <li>{ToolCallNode} 调用 {@link #recordToolObservation} 提取环境/软件/失败信号</li>
 *   <li>{AiCallNode} 调用 {@link #recordAssistantConclusion} 提取排查结论</li>
...
public interface ILongTermMemoryService {
     ...
     * <p>
     * Case 层（RootNode）冷启动时通过此方法加载历史消息，不再直接调用仓储层。
     *
     * @param sessionId 会话 ID
     * @param limit     最大返回条数
